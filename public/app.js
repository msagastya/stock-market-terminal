...
const state = {
  ...
  chartDataCache: {}, // Cache for all historical chart timelines
...
const elements = {
  ...
  // Custom Chart Type buttons
  btnChartTypeLine: document.getElementById('btn-chart-type-line'),
  btnChartTypeCandles: document.getElementById('btn-chart-type-candles'),

  // Moat Badge in Stock Hero
  stockMoatBadge: document.getElementById('stock-moat-badge'),

  // Fundamentals Dashboard
  fundGrossMargin: document.getElementById('fund-gross-margin'),
  fundGrossMarginBar: document.getElementById('fund-gross-margin-bar'),
  fundOperatingMargin: document.getElementById('fund-operating-margin'),
  fundOperatingMarginBar: document.getElementById('fund-operating-margin-bar'),
  fundNetMargin: document.getElementById('fund-net-margin'),
  fundNetMarginBar: document.getElementById('fund-net-margin-bar'),
...
async function loadStock(symbol, silent = false) {
  ...
  try {
    ...
    // 3.5. Update AI Executive Verdict & Moat Analysis
...
async function preloadAllChartRanges(symbol) {
  ...
  // Background pre-fetch in parallel
  Promise.allSettled(ranges.map(async (range) => {
    if (!state.chartDataCache[symbol][range]) {
      try {
        state.chartDataCache[symbol][range] = await api.getHistoricalData(symbol, range);
      } catch (err) {
        console.warn(`Failed to pre-fetch range ${range} for ${symbol}:`, err.message);
      }
    }
  })).then(() => {
    console.log(`Preloaded all timelines for ${symbol}`);
  });
}
...
function updateFundamentalsAndMoat(quote) {
  ...
  const grossMarginVal = quote.grossMargins !== undefined && quote.grossMargins !== null ? quote.grossMargins * 100 : null;
  const operatingMarginVal = quote.operatingMargins !== undefined && quote.operatingMargins !== null ? quote.operatingMargins * 100 : null;
  const netMarginVal = quote.profitMargins !== undefined && quote.profitMargins !== null ? quote.profitMargins * 100 : null;
  const roeVal = quote.returnOnEquity !== undefined && quote.returnOnEquity !== null ? quote.returnOnEquity * 100 : null;
  const roaVal = quote.returnOnAssets !== undefined && quote.returnOnAssets !== null ? quote.returnOnAssets * 100 : null;
  const debtEquityVal = quote.debtToEquity !== undefined && quote.debtToEquity !== null ? quote.debtToEquity : null;
  const fcfVal = quote.freeCashFlow;

  if (elements.fundGrossMargin && elements.fundGrossMarginBar) {
    if (grossMarginVal !== null) {
      elements.fundGrossMargin.textContent = `${grossMarginVal.toFixed(2)}%`;
      elements.fundGrossMarginBar.style.width = `${Math.max(0, Math.min(100, grossMarginVal))}%`;
    } else {
      elements.fundGrossMargin.textContent = 'N/A';
      elements.fundGrossMarginBar.style.width = '0%';
    }
  }

  if (elements.fundOperatingMargin && elements.fundOperatingMarginBar) {
    if (operatingMarginVal !== null) {
      elements.fundOperatingMargin.textContent = `${operatingMarginVal.toFixed(2)}%`;
      elements.fundOperatingMarginBar.style.width = `${Math.max(0, Math.min(100, operatingMarginVal))}%`;
    } else {
      elements.fundOperatingMargin.textContent = 'N/A';
      elements.fundOperatingMarginBar.style.width = '0%';
    }
  }

  if (elements.fundNetMargin && elements.fundNetMarginBar) {
    if (netMarginVal !== null) {
      elements.fundNetMargin.textContent = `${netMarginVal.toFixed(2)}%`;
      elements.fundNetMarginBar.style.width = `${Math.max(0, Math.min(100, netMarginVal))}%`;
    } else {
      elements.fundNetMargin.textContent = 'N/A';
      elements.fundNetMarginBar.style.width = '0%';
    }
  }

  if (elements.fundRoe) {
    if (roeVal !== null) {
      elements.fundRoe.textContent = `${roeVal.toFixed(2)}%`;
      elements.fundRoe.className = `badge ${roeVal > 15 ? 'badge-positive' : roeVal > 8 ? 'badge-neutral' : 'badge-negative'}`;
    } else {
      elements.fundRoe.textContent = 'N/A';
      elements.fundRoe.className = 'badge badge-neutral';
    }
  }

  if (elements.fundRoa) {
    if (roaVal !== null) {
      elements.fundRoa.textContent = `${roaVal.toFixed(2)}%`;
      elements.fundRoa.className = `badge ${roaVal > 8 ? 'badge-positive' : roaVal > 4 ? 'badge-neutral' : 'badge-negative'}`;
    } else {
      elements.fundRoa.textContent = 'N/A';
      elements.fundRoa.className = 'badge badge-neutral';
    }
  }

  if (elements.fundDebtEquity) {
    if (debtEquityVal !== null) {
      const ratio = debtEquityVal > 3 ? debtEquityVal / 100 : debtEquityVal;
      elements.fundDebtEquity.textContent = `${ratio.toFixed(2)}x`;
      elements.fundDebtEquity.className = `badge ${ratio < 0.5 ? 'badge-positive' : ratio < 1.5 ? 'badge-neutral' : 'badge-negative'}`;
    } else {
      elements.fundDebtEquity.textContent = 'N/A';
      elements.fundDebtEquity.className = 'badge badge-neutral';
    }
  }

  if (elements.fundFcf) {
    if (fcfVal !== null && fcfVal !== undefined) {
      elements.fundFcf.textContent = formatLargeNumber(fcfVal);
    } else {
      elements.fundFcf.textContent = 'N/A';
    }
  }

  let moatScore = 0;
  let moatRating = 'NO MOAT';
  let moatBadgeClass = 'badge-neutral';
  let moatColor = '#94a3b8';

  const dRatio = debtEquityVal !== null ? (debtEquityVal > 3 ? debtEquityVal / 100 : debtEquityVal) : null;

  const isWideMargin = operatingMarginVal !== null && operatingMarginVal >= 20;
  const isWideRoe = roeVal !== null && roeVal >= 15;
  const isSafeDebt = dRatio === null || dRatio < 0.5;

  const isNarrowMargin = operatingMarginVal !== null && operatingMarginVal >= 10;
  const isNarrowRoe = roeVal !== null && roeVal >= 10;

  if (isWideMargin && isWideRoe && isSafeDebt) {
    moatRating = 'WIDE MOAT';
    moatScore = 5.0;
    moatBadgeClass = 'badge-positive';
    moatColor = '#00f5a0';
  } else if (isNarrowMargin || isNarrowRoe) {
    moatRating = 'NARROW MOAT';
    moatScore = 3.0;
    moatBadgeClass = 'badge-neutral';
    moatColor = '#3b82f6';
  } else {
    moatRating = 'NO MOAT';
    moatScore = 1.0;
    moatBadgeClass = 'badge-negative';
    moatColor = '#ff0055';
  }

  if (elements.stockMoatBadge) {
    elements.stockMoatBadge.textContent = moatRating;
    elements.stockMoatBadge.className = `badge-meta badge-moat ${moatBadgeClass}`;
    elements.stockMoatBadge.style.color = moatColor;
    elements.stockMoatBadge.style.borderColor = moatColor;
    elements.stockMoatBadge.style.background = `linear-gradient(135deg, ${moatColor}15, ${moatColor}05)`;
  }

  if (elements.moteActiveSymbol) {
    elements.moteActiveSymbol.textContent = quote.symbol;
  }

  if (elements.moteScoreBadge) {
    elements.moteScoreBadge.textContent = `${moatScore.toFixed(1)} / 5.0 Moat`;
    elements.moteScoreBadge.className = `badge ${moatBadgeClass}`;
  }

  if (elements.moteStarsDisplay) {
    const filledStars = '★'.repeat(Math.round(moatScore));
    const emptyStars = '☆'.repeat(5 - Math.round(moatScore));
    elements.moteStarsDisplay.textContent = filledStars + emptyStars;
  }

  const checklistQuestions = {
    'mote-q-brand': isWideMargin || (quote.recommendation && quote.recommendation.toLowerCase().includes('buy')),
    'mote-q-switching': isNarrowRoe && isNarrowMargin,
    'mote-q-network': isWideMargin && isWideRoe,
    'mote-q-cost': isWideMargin && isSafeDebt,
    'mote-q-barriers': isWideMargin && quote.marketCap > 100000000000
  };

  elements.moteCheckboxes.forEach(cb => {
    if (checklistQuestions[cb.id] !== undefined) {
      cb.checked = checklistQuestions[cb.id];
    }
  });
}
...
function setupListeners() {
  ...
  // Chart Type Selection (Line vs Candles)
  if (elements.btnChartTypeLine && elements.btnChartTypeCandles) {
    elements.btnChartTypeLine.addEventListener('click', () => {
      elements.btnChartTypeLine.classList.add('active');
      elements.btnChartTypeCandles.classList.remove('active');
      chartManager.setChartType('line');
    });
    elements.btnChartTypeCandles.addEventListener('click', () => {
      elements.btnChartTypeCandles.classList.add('active');
      elements.btnChartTypeLine.classList.remove('active');
      chartManager.setChartType('candlestick');
    });
  }

  // Moat Badge Click Action
  if (elements.stockMoatBadge) {
    elements.stockMoatBadge.addEventListener('click', () => {
...
function showGithubPagesBackendAlert() {
  ...
  alertDiv.innerHTML = `
    <div style="display:flex; align-items:center; gap:0.5rem; font-size:0.9rem;">
      <span>⚠️</span>
      <span><strong>AuraStock hosted terminal:</strong> Connect your local Node.js backend. Use your local network IP (e.g. <code>http://192.168.x.x:3000</code>) if accessing from another device (mobile/tablet).</span>
    </div>
